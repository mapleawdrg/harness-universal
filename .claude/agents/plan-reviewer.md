...
2. **설계 결함 시사 (architect 영역으로 회귀)**: 아래 신호 중 하나 이상
   ...
   > **Iteration 카운터 처리**: architect 재정의가 PASS되면 다음 planner 호출은 **새 라운드**로 간주한다. plan-review-p{PHASE}.md를 새 파일로 시작하거나 기존 파일의 Iteration을 N=1로 리셋한다. (이전 N=3 누적 카운터를 그대로 들고 가면 즉시 종료 트랩)
