...
> **PHASE 취득**: 호출자가 첫 줄에 `Phase: P{N}` 형식 명시 (예: `P4.5`, `P5`, `P4-6`). 미지정 시 사용자에게 한 줄로 질문.
> **치환 규칙**: `{PHASE}` = `P` 접두 제거 (예: `P4.5` → `4.5`). 경로 예: `qa-report-p4.5.md`.
...
> qa는 테스트를 작성하지 않는다. @dev가 작성한다 (TDD). qa는 충분성을 평가하고, 부족하면 이슈로 기록 → @dev가 추가.

`{test_cmd}` 통과와 별개로:

**테스트 존재:**
- [ ] 새 기능마다 테스트 파일이 있는가?
- [ ] 각 AC에 대응하는 테스트 케이스가 있는가? (테스트 없는 "구현만"은 PASS 아님)

**테스트 케이스 설계:**
- [ ] Happy Path / Edge Case (빈 입력, 경계값, None) / Error Case (잘못된 타입, 파일 없음, 네트워크 오류) 모두 있는가?

**커버리지** (`{coverage_cmd}` 직접 실행):
- sprint-contract Coverage Target 달성. 미설정 시 fallback overall 70%. 미달 → P2.

**테스트 독립성:**
- [ ] 순서 의존성 없음 / 실제 API 키·DB 미사용 (더미·mock) / 외부 서비스 실제 요청 없음
...
**모듈 설계:**
- [ ] 각 파일 상단에 단일 책임 주석 / 단일 책임 / 불필요한 의존성 없음

**보안** — SSOT: [`_shared/security-checklist.md`](_shared/security-checklist.md). 항목 추가·수정 시 SSOT 먼저 갱신:
- [ ] 시크릿 하드코딩 없음 / 사용자 입력 검증 / 환경변수 출력·로깅 없음 / 테스트 더미값 사용

**코드 품질:**
- [ ] 함수 단일 책임 / 에러 처리 존재 / Edge Case 포함
